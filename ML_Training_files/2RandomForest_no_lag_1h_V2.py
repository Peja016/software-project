...
# 2.3 Lag feature (only keep lag_24h)
...
plt.ylabel('Feature')
plt.title('Feature Importance in Random Forest Model (No lag_1h)')
plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig('feature_importance_no_lag_1h.png')

# 9. Save the model
joblib.dump(model, "bike_availability_rf_model_no_lag_1h.joblib", compress=3)
with open("bike_availability_rf_model_no_lag_1h.pkl", "wb") as f:
    pickle.dump(model, f)

# 10. Calculate default lag values (only lag_24h)
default_lags = data.groupby(['station_id', 'hour']).agg({
    'lag_24h': 'mean'
}).reset_index()
# Fill missing values (using the station's overall mean)
default_lags['lag_24h'] = default_lags['lag_24h'].fillna(default_lags.groupby('station_id')['lag_24h'].transform('mean'))
default_lags.to_csv("default_lags.csv", index=False)

# 11. Generate station information table
...
print("Feature importance chart saved as feature_importance_no_lag_1h.png")
